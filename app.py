...
history_df = pd.DataFrame(columns=["Stocks %", "Cash %", "Bonds %", "Investment Style"])
...
slider_card = dbc.Card(
    [
        ...
        dbc.Button("Previous Setting", id="previous-setting-btn", n_clicks=0, color="primary"),
...
tabs = dbc.Tabs(
    [
        ...
        dbc.Tab(label="History",
            tab_id="tab-1",
            children=[
                html.Div(
                    id="history-table-container",
                    children=[
                        dcc.Store(id="history-store", data={"records": []}),
...
def backtest(stocks, cash, start_bal, nper, start_yr):
    ...
    for yr in dff.Year + 1:
        if yr <= end_yr:
            ...
            dff.loc[yr, "Total"] = int(dff.loc[yr, ["Cash", "Bonds", "Stocks"]].sum().round())
...
def update_bar(stocks, cash, history_data):
    ...
    figure = go.Figure(
        data=[
            go.Bar(
                ...
                text=[f"{v}%" for v in slider_input],
                textposition="auto",
            )
        ]
    )
    ...
    figure.update_layout(
        title_text=f"{investment_style} Asset Allocation",
        title_x=0.5,
        yaxis=dict(title="Percentage", range=[0, 100], fixedrange=True),
    ...
    history_data["records"].append(new_entry)

    return figure, bonds, history_data
...
def update_history_table(history_data):
    if not history_data["records"]:
        return html.Div("No history available yet.", style={"padding": "10px"})

    # Convert to DataFrame
    history_df = pd.DataFrame(history_data["records"])
